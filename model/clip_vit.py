...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, device):
        ...
        self.encoder, _ = clip.load("/home/B/lws/code/python/IRS/pretrain_model/ViT-B-16.pt",
                                    device=device)  # loads already in eval mode
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            x = torch.cat([self.encoder.visual.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1],
                                                                                         dtype=x.dtype,
                                                                                         device=x.device), x],
