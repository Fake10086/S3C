...
from dataset.clipcap_train import prompt_text, load_data
...
def main():
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(len(dataset))):
            ...
            if cfg.beam_search:
                generated_text = generate_beam(model, tokenizer, embed, device, beam_size=5, return_top_pred=True,
                                               entry_length=entry_length, stop_token_index=tokenizer.eos_token_id)
                gen[q] = []
                gen[q].append({
                    "question_id": q,
                ...
                generated_text = torch.tensor(dataset.tokenizer(generated_text + ' ' + 'answer:')['input_ids'],
                                              dtype=torch.long).cuda()
                generated_text = model.gpt.transformer.wte(generated_text).unsqueeze(0)
                e = torch.cat([embed, generated_text], dim=1)
                answer = generate_beam(model, tokenizer, e, device, beam_size=3, return_top_pred=True,
                                       entry_length=entry_length, stop_token_index=tokenizer.eos_token_id)
    ...
    with open(f"{cfg.aokvqa_dir}/aokvqa_v1p0_val.json", 'r') as f:
        data = json.load(f)
    gts = {}
    ...
    for i in data:
        ...
        for j in i["rationales"]:
            gts[i['question_id']].append({
                "question_id": i['question_id'],
                "caption": j,
            })

    cocoEval = COCOEvalCap(gts, gen)
    cocoEval.evaluate()
